...
API_KEY = os.getenv("API_KEY") or os.getenv("AIPIPE_TOKEN")  # Get API key from environment variable
...
if not API_KEY:
    logger.error("API_KEY or AIPIPE_TOKEN environment variable is not set. The application will not function correctly.")
    logger.error("Please set either API_KEY (OpenAI) or AIPIPE_TOKEN (aipipe) in your .env file.")
...
async def get_embedding(text, max_retries=3):
    if not API_KEY:
        error_msg = "API_KEY or AIPIPE_TOKEN environment variable not set"
        logger.error(error_msg)
        raise HTTPException(status_code=500, detail=error_msg)
    ...
    while retries < max_retries:
        try:
            ...
            if API_KEY.startswith("sk-"):
                ...
                headers = {
                    "Authorization": API_KEY,  # aipipe uses direct token
                    "Content-Type": "application/json"
                }
...
async def query_knowledge_base(request: QueryRequest):
    try:
        # Log the incoming request        logger.info(f"Received query request: question='{request.question[:50]}...', image_provided={request.image is not None}")
