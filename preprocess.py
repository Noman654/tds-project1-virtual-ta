...
# Get API key from environment variable (supports both OpenAI and aipipe tokens)
API_KEY = os.getenv("API_KEY") or os.getenv("AIPIPE_TOKEN")
if not API_KEY:
    logger.error("API_KEY or AIPIPE_TOKEN environment variable not set.")
    logger.error("For OpenAI: Set API_KEY with a key starting with 'sk-' from https://platform.openai.com/api-keys")
    logger.error("For aipipe: Set AIPIPE_TOKEN with your aipipe JWT token")
...
elif "." in API_KEY and len(API_KEY.split(".")) == 3:
    logger.info("aipipe JWT token loaded successfully")
else:
    logger.warning("API key format not recognized. Continuing anyway...")
    logger.info(f"API key loaded (starts with: {API_KEY[:10]}...)")
            ...
            try:
                ...
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(f"Authentication failed (status 401): {error_text}")
                        logger.error("Your API key appears to be invalid. Please check:")
                        logger.error("1. The API key is correctly set in your .env file")
                        if api_key.startswith("sk-"):
                            logger.error("2. The OpenAI API key starts with 'sk-proj-' or 'sk-'")
                            logger.error("3. The API key has not expired")
                            logger.error("4. You have sufficient credits in your OpenAI account")
                        else:
                            logger.error("2. The aipipe token is valid and not expired")
                            logger.error("3. You have access to the aipipe service")
                        return False
                    else:
                        error_text = await response.text()
...
async def main():
    ...
    parser.add_argument("--api-key", help="API key for OpenAI or aipipe proxy (if not provided, will use API_KEY or AIPIPE_TOKEN environment variable)")
    ...
    if not api_key:
        logger.error("API key not provided. Please provide it via --api-key argument or set API_KEY/AIPIPE_TOKEN environment variable.")
        logger.error("For OpenAI: Set API_KEY with a key starting with 'sk-'")
        logger.error("For aipipe: Set AIPIPE_TOKEN with your JWT token")
        return
